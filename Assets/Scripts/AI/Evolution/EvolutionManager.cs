...
public class EvolutionManager : MonoBehaviour
{
    ...
    public void StartEvolution()
    {
        ...
        if (SaveStatistics)
        {
            statisticsFileName = "Evaluation - Main " + DateTime.Now.ToString("yyyy_MM_dd_HH-mm-ss");
            //TODO
    ...
    private void WriteStatisticsFileStart()
    {
        File.WriteAllText(statisticsFileName + ".txt", "Evaluation of a Population with size " + PopulationSize +
                ", on Track \"Main\", using the following GA operators: " + Environment.NewLine +
    ...
    private void StartEvaluation(IEnumerable<Genotype> currentPopulation)
    {
        ...
        IEnumerator<CarManager> carsEnumerator = TrackManager.Instance.GetCarEnumerator();
        for (int i = 0; i < agents.Count; i++)
        {
            if (!carsEnumerator.MoveNext())
            {
                Debug.LogError("Cars enum ended before agents.");
                break;
            }

            carsEnumerator.Current.Agent = agents[i];
            AgentsAliveCount++;
        ...
        Debug.Log("Restarting: " + AgentsAliveCount + " agents remaining");
        Debug.Log("Actualy there are: " + agents.Count(a => a.IsAlive) + " agents remaining");
    }
    ...
    private void OnAgentDied(Agent agent)
    {
        ...
        //Bugfix because when we allready do -- this shouldn't be possible (not sure why this happens but this fixes the issue)
        while (agents.Count == AgentsAliveCount)
        {
            AgentsAliveCount--;
        }

        //Debug.Log("Agent died, " + AgentsAliveCount + " remaining from " + agents.Count);
    }
